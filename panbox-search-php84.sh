...
# ============================================
# Panbox-Search PHP 8.4 - 一键部署脚本
# ============================================

set -e

SCRIPT_VERSION="2026.07.06.1"
SELF_UPDATE_RESTARTED_ENV="PANBOX_SEARCH_PHP84_SCRIPT_SELF_UPDATED"
SCRIPT_URLS=(
    "https://gh-proxy.org/https://raw.githubusercontent.com/kokojacket/panbox-search-deploy/main/panbox-search-php84.sh"
    "https://hk.gh-proxy.org/https://raw.githubusercontent.com/kokojacket/panbox-search-deploy/main/panbox-search-php84.sh"
    "https://cdn.gh-proxy.org/https://raw.githubusercontent.com/kokojacket/panbox-search-deploy/main/panbox-search-php84.sh"
    "https://edgeone.gh-proxy.org/https://raw.githubusercontent.com/kokojacket/panbox-search-deploy/main/panbox-search-php84.sh"
    "https://raw.githubusercontent.com/kokojacket/panbox-search-deploy/main/panbox-search-php84.sh"
)
PANBOX_DIR="/opt/panbox-search-php84"
COMPOSE_FILE="docker-compose.yml"
DEFAULT_IMAGE="kokojacket/panbox-search:php-8.4"
DEFAULT_POLLER_IMAGE="kokojacket/panbox-openilink-poller:php-8.4"
COMPOSE_CMD=""
NEED_SUDO=false
...
check_and_force_self_update() {
    ...
    if [ "$remote_version" = "$SCRIPT_VERSION" ]; then
        rm -f "$tmp_file"
        success "PHP 8.4 部署脚本已是最新版本：$SCRIPT_VERSION"
        return 0
    fi

    warning "检测到 PHP 8.4 部署脚本更新：当前 $SCRIPT_VERSION → 最新 $remote_version"
...
REDIS_PREFIX=${REDIS_PREFIX:-panbox-php84:}
...
services:
  app:
    image: ${PANBOX_IMAGE:-kokojacket/panbox-search:php-8.4}
    container_name: panbox-search-php84-app
    ports:
      - "${APP_PORT:-8094}:80"
    environment:
      - DB_HOST=mysql
      ...
      - SYSTEM_SALT=Panbox Search PHP 8.4
      - SITE_NAME=Panbox Search PHP 8.4
      - CACHE_DRIVER=${CACHE_DRIVER:-redis}
      - REDIS_HOST=redis
      - REDIS_PORT=6379
      - REDIS_PASSWORD=${REDIS_PASSWORD:-}
      ...
      - APACHE_MAX_SPARE_SERVERS=${APACHE_MAX_SPARE_SERVERS:-8}
    volumes:
      - /opt/panbox-search-php84/app/runtime:/var/www/html/runtime
      - /opt/panbox-search-php84/app/data:/var/www/html/data
      - /opt/panbox-search-php84/app/uploads:/var/www/html/public/uploads
      - /opt/panbox-search-php84/app/install:/var/www/html/public/install
    depends_on:
      - mysql
      - redis
    networks:
      - panbox-search-php84-network
    restart: unless-stopped

  mysql:
    image: mysql:5.7
    container_name: panbox-search-php84-mysql
    environment:
      ...
      - TZ=Asia/Shanghai
    volumes:
      - /opt/panbox-search-php84/mysql:/var/lib/mysql
    networks:
      - panbox-search-php84-network
    restart: unless-stopped
    command:
  ...
  openilink-poller:
    image: ${PANBOX_POLLER_IMAGE:-kokojacket/panbox-openilink-poller:php-8.4}
    container_name: panbox-search-php84-openilink-poller
    environment:
      - MYSQL_HOST=mysql
      - MYSQL_PORT=3306
      - MYSQL_DATABASE=panbox-search
  ...
  redis:
    ...
    container_name: panbox-search-php84-redis
    command: >
    ...
    environment:
      - REDIS_PASSWORD=${REDIS_PASSWORD:-}
    volumes:
      - /opt/panbox-search-php84/redis:/data
    networks:
      - panbox-search-php84-network
    restart: unless-stopped
...
install_system() {
    log "开始安装 Panbox Search PHP 8.4..."
    check_docker
    check_docker_permissions
    check_docker_compose
    create_directories
...
update_system() {
    log "开始更新 Panbox Search PHP 8.4..."
    check_docker
    check_docker_permissions
    check_docker_compose
    create_directories
...
manage_service() {
    ...
    if [ ! -f "$PANBOX_DIR/$COMPOSE_FILE" ]; then
        error "未找到 PHP 8.4 安装目录：$PANBOX_DIR"
        exit 1
    fi
...
show_info() {
    ...
    success "Panbox Search PHP 8.4 已启动"
